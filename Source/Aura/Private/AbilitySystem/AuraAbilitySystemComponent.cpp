...
void UAuraAbilitySystemComponent::AddCharacterAbilitiesFromSaveData(UAuraLoadScreenSaveGame* SaveData)
{
...
{
	FScopedAbilityListLock ActiveScopeLock(*this);
	for (const FGameplayAbilitySpec& Abnilityspec : GetActivatableAbilities())
	{
		if (!Delegate.ExecuteIfBound(Abnilityspec))
		{
...
bool UAuraAbilitySystemComponent::IsPasiveAbility(const FGameplayAbilitySpec& Spec) const
{
...
void UAuraAbilitySystemComponent::UpgradeAttribute(const FGameplayTag& AttributeTag)
{
	if (GetAvatarActor()->Implements<UPlayerInterface>())
	{
		...
		{
			ServerUpgradeAtribute(AttributeTag);
		}
	}
}

void UAuraAbilitySystemComponent::ServerUpgradeAtribute_Implementation(const FGameplayTag& AttributeTag)
{
		...
		if (bStatusValid)
		{
			...
			if (!SlotIsEmpty(Slot)) // There is an ability in this slot already. Deactivate and clear its slot
			{
				...
				if (SpecWithSlot)
				{
					...
					if (IsPasiveAbility(*SpecWithSlot))
					{
			...
			{
				if (IsPasiveAbility(*AbilitySpec))
				{
					TryActivateAbility(AbilitySpec->Handle);
