...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		// Debuff
		if (UAuraAbilitySystemLibrary::IsSucccsessfulDebuff(Props.EffectContextHandle))
		{
			Debuff(Props);
		}
		// LifeSiphon 
		if (Props.SourceASC && Props.SourceASC->HasMatchingGameplayTag(GameplayTags.Abilities_Passive_LifeSiphon))
		{
			Siphon("Life", LocalIncomingDamage, Props);
		}

		// ManaSiphon 
		if (Props.SourceASC && Props.SourceASC->HasMatchingGameplayTag(GameplayTags.Abilities_Passive_ManaSiphon))
		{
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectProperties& Props)
{
	...
	// Source Character is the owner, since GA_ListenForEvents applies GE-EventBasedEffect, adding to IncomingXP
	if (Props.SourceCharacter->Implements<UPlayerInterface>() && Props.SourceCharacter->Implements<UCombatInterface>())
	{
		const int32 CurrentLevel = ICombatInterface::Execute_GetPlayerLevel(Props.SourceCharacter);
		const int32 CurrentXP = IPlayerInterface::Execute_GetoXP(Props.SourceCharacter);
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	if (!EffectClass)
	{
		UE_LOG(LogTemp, Error, TEXT("Could not load GameplayEffect class at path: %s"), *Path);
		return;
	}

	UGameplayEffect* Effect = NewObject<UGameplayEffect>(GetTransientPackage(), EffectClass, FName(DebuffName));
	...
	if (FGameplayEffectSpec* MutableSopec = new FGameplayEffectSpec(Effect, EffectContext, 1.f))
	{
		FAuraGameplayEffectContext*  AuraContext = static_cast<FAuraGameplayEffectContext*>(MutableSopec->GetContext().Get());
		TSharedPtr<FGameplayTag> DebuffDamageType = MakeShareable(new FGameplayTag(DamageType));
		AuraContext->SetDamageType(DebuffDamageType);
		
		Props.TargetASC->ApplyGameplayEffectSpecToSelf(*MutableSopec);
	}
}
...
void UAuraAttributeSet::Siphon(const FString& Attribute, float Damage, const FEffectProperties& Props)
{
	...
	UGameplayEffect* Effect = NewObject<UGameplayEffect>(GetTransientPackage(), EffectClass, FName(SiphonName));

    const FGameplayTag SiphonTag = FGameplayTag::RequestGameplayTag(FName(FString::Printf(TEXT("Abilities.Passive.%s"), *SiphonName)));
